#!/usr/bin/env python3
"""Interactive uploader for recorded hexapod sessions on Hugging Face."""
...
DEFAULT_EPISODES_ROOT = REPO_ROOT / "data" / "hexapod_cnn" / "episodes"
LEGACY_EPISODES_ROOT = REPO_ROOT / "data" / "hexapod_cnn_loop" / "episodes"
...
def sanitize_repo_name(value: str) -> str:
    ...
    return cleaned or "hexapod-session"
...
def repo_card_text(summary: SessionSummary, repo_id: str, include_raw: bool) -> str:
    ...
    return textwrap.dedent(
        ...
        This dataset repo was uploaded from the hexapod recording stack.
        ...
        This repo contains one selected hexapod session only. The repo name intentionally matches the session folder name.
        """
    ).strip() + "\n"
...
def stage_upload_folder(summary: SessionSummary, *, repo_id: str, include_raw: bool) -> Path:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="hexapod_hf_upload_"))
    ...
    *,
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_type="dataset",
            token=token,
            commit_message=f"Upload hexapod session {summary.session_name}",
            commit_description=(
...
class SessionUploaderApp:
    ...
    def _build(self) -> None:
        self.root.title("hexapod Hugging Face Session Uploader")
